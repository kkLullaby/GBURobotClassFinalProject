...
- [2026-06-19-m4-implement-show-emoji-and-text-024](active/2026-06-19-m4-implement-show-emoji-and-text-024.md) planner → executor (pending, **可交 coding agent / sandbox-friendly write 部分**): "M4 真实现：otto_controller.cc::RegisterMcpTools() 加 show_emoji + show_text 2 个 AddTool (~50 LOC)" — H023 verdict 落地；用户后续 flash + 端到端测 H024.bis；Week 2 §2.1-2.3 主线
- [2026-06-19-m2-hermes-hmac-sign-025](active/2026-06-19-m2-hermes-hmac-sign-025.md) planner → executor (pending, **可交 coding agent / sandbox-friendly**): "shim HermesBackend 加 X-Hub-Signature-256 HMAC 签名 (~5 行 + 2 测)" — H020 实测发现 Hermes 强制 HMAC；本 handoff 让 fork 真进 Hermes session
- [2026-06-19-week0-retro-026](active/2026-06-19-week0-retro-026.md) planner → planner (pending, ~1h self-work): "写 docs/retros/week0.md (~300-400 行 §1-§6)" — Week 0 集中复盘 + 收纳 bitter lessons + agent-arch 实战回顾；答辩素材
