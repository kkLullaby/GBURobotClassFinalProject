...
- [2026-06-20-m3-plugin-install-and-hermes-verify-028bis](active/2026-06-20-m3-plugin-install-and-hermes-verify-028bis.md) planner → **user** (pending, ~15-20min 物理): "M3 Stage B：cp plugin → ~/.hermes/plugins/xiaozhi/ + hermes gateway run 看注册日志 + 切 shim env 指 8645 webhook + ESP32 voice 真路径 → TUI sidebar 见 [xiaozhi] channel" — 验 plugin loader 真认我们包；H028 mock 已 PASS, 但 hermes loader 行为只能真启验
- [2026-06-20-m3-m1-proxy-and-real-send-029](active/2026-06-20-m3-m1-proxy-and-real-send-029.md) planner → executor (pending, **可交 coding agent / sandbox-friendly**, ~1 天): "M3 Stage C：M1 加 show_text_proxy stdio MCP tool + sidecar HTTP /tools/show_text + adapter.send 真接 M1 → ESP32" — H028 桩已铺好；本 handoff 让 Hermes 真能推文字到机器人屏幕；M1(3 new) + M3(1 new) = 4 pytest case；无新依赖外 fastapi (M2 已装齐 conda env 有)

并行 backlog（不阻塞 M3 三件套）：
- H024.bis (user 物理): flash ESP32 + tool count 14 + "笑一个" 验
- H027 (planner self, ~30min): docs 集中回灌 8 项 + 加 H028 bitter lesson #17 (per [retro §6](../retros/week0.md))
